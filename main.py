...
from training import TransitionDataset, adversarial_imitation_update, behavioral_cloning_update, compute_advantages, ppo_update, target_estimation_update
...
parser.add_argument('--value-loss-coeff', type=float,
                    default=0.5, metavar='c1', help='Value loss coefficient')
...
parser.add_argument('--imitation', type=str, default='', choices=[
                    'AIRL', 'BC', 'GAIL', 'GMMIL', 'RED'], metavar='I', help='Imitation learning algorithm')
...
if args.imitation:
  ...
  if args.imitation in ['AIRL', 'GAIL', 'GMMIL', 'RED']:
    if args.imitation == 'AIRL':
      ...
      discriminator = REDDiscriminator(
          env.observation_space.shape[0], env.action_space.n, args.hidden_size, state_only=args.state_only)
    ...
    if step == 1:
      for _ in tqdm(range(args.imitation_epochs), leave=False):
        if args.imitation == 'BC':
          ...
          target_estimation_update(
              discriminator, expert_trajectories, discriminator_optimizer, args.imitation_batch_size)
          ...
          with torch.no_grad():
            if args.imitation == 'AIRL':
              policy_trajectories['rewards'] = discriminator.predict_reward(policy_trajectories['states'], policy_trajectories['actions'], torch.cat(
                  [policy_trajectories['states'][1:], next_state]), policy_trajectories['log_prob_actions'].exp(), policy_trajectories['terminals'])
            elif args.imitation == 'GAIL':
